...
bool TextureManager::Load(std::string id, std::string fileName)
{
	...
	SDL_Texture* texture =SDL_CreateTextureFromSurface(Engine::GetInstance()->GetRenderer(), surface);//cargar la textura
...
void TextureManager::Draw(std::string id, int x, int y, int width, int height, SDL_RendererFlip flip)//para dibujar la imagen en la pantalla
{
	...
	SDL_RenderCopyEx(Engine::GetInstance()->GetRenderer(), m_TextureMap[id], &srcRect, &destRect, 0, nullptr, flip);//PARA TENER MAS COMPONENTES COMO EL ANGULO del pichingo y asi y poder rtarlo

}

void TextureManager::DrawFrame(std::string id, int x, int y, int width, int height, int row, int frame, SDL_RendererFlip flip)//para dibujar la imagen en la pantalla
{
